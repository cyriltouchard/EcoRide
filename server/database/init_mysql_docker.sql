-- ================================================
-- SCHEMA SQL ECORIDE - Version Docker
-- ================================================
...
DELIMITER //
CREATE TRIGGER IF NOT EXISTS tr_update_credits
    AFTER INSERT ON credit_transactions
    FOR EACH ROW
BEGIN
    IF NEW.transaction_type = 'gain' THEN
        UPDATE user_credits 
        SET current_credits = current_credits + NEW.amount,
            total_earned = total_earned + NEW.amount,
            last_transaction = NOW()
        WHERE user_id = NEW.user_id;
    ELSEIF NEW.transaction_type IN ('depense', 'commission') THEN
        UPDATE user_credits 
        SET current_credits = current_credits - NEW.amount,
            total_spent = total_spent + NEW.amount,
            last_transaction = NOW()
        WHERE user_id = NEW.user_id;
    ELSEIF NEW.transaction_type = 'remboursement' THEN
        UPDATE user_credits 
        SET current_credits = current_credits + NEW.amount,
            last_transaction = NOW()
        WHERE user_id = NEW.user_id;
